...
class TransactionUtil extends Util
{
            ...
            if (! empty($payment['payment_id'])) {
                ...
                if ($payment_amount != 0) {
                    ...
                    if (! empty($payment['paid_on'])) {
                        if ($uf_data) {
                            // Try to parse the date, if it fails, use current date
                            try {
                                // Check if it's already in datetime format
                                if (preg_match('/^\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}$/', $payment['paid_on'])) {
                                    $paid_on = $payment['paid_on'];
                                } else {
                                    $paid_on = $this->uf_date($payment['paid_on'], true);
                                }
                            } catch (\Exception $e) {
                                // If parsing fails, try to parse as Carbon
                                try {
                                    $paid_on = \Carbon::parse($payment['paid_on'])->toDateTimeString();
                                } catch (\Exception $e2) {
    ...
    /**
     * Get profit from direct sales (sales without products or with is_direct_sale flag)
     * For direct sales, the entire sale amount is considered as profit since there's no cost of goods
     *
     * @param  int  $business_id
     * @param  string  $start_date = null
     * @param  string  $end_date = null
     ...
     * @param  array  $permitted_locations = null
     * @return float
     */
    public function getDirectSalesProfit($business_id, $start_date = null, $end_date = null, $location_id = null, $user_id = null, $permitted_locations = null)
    {
        ...
        // Get sales that either have is_direct_sale flag OR have no sell lines (no products)
        // Use left join to check if sales have sell lines
        $query->leftJoin('transaction_sell_lines as tsl', function($join) {
            $join->on('transactions.id', '=', 'tsl.transaction_id');
        })
        ->select(
            'transactions.*',
            DB::raw('COUNT(tsl.id) as sell_lines_count')
        )
        ->groupBy('transactions.id');

        $direct_sales = $query->get();

        $direct_sales_profit = 0;
        foreach ($direct_sales as $sale) {
            // Check if it's a direct sale (has is_direct_sale flag) or has no sell lines
            if ($sale->is_direct_sale || $sale->sell_lines_count == 0) {
                // For direct sales, entire sale amount (excluding tax) is profit
                // Subtract any discounts and add round off
                $sale_profit = $sale->total_before_tax;
                
                // Subtract discount if any
                if ($sale->discount_type == 'percentage') {
                    $sale_profit = $sale_profit - ($sale_profit * ($sale->discount_amount / 100));
                } elseif ($sale->discount_type == 'fixed') {
                    $sale_profit = $sale_profit - ($sale->discount_amount ?? 0);
                }
                
                // Add round off if any
                $sale_profit = $sale_profit + ($sale->round_off_amount ?? 0);
                
                $direct_sales_profit += $sale_profit;
            }
        }

        return $direct_sales_profit;
    }
    ...
    public function getProfitLossDetails($business_id, $location_id, $start_date, $end_date, $user_id = null, $permitted_locations = null)
    {
        ...
        // Get direct sales profit (sales without products or with is_direct_sale flag)
        $direct_sales_profit = $this->getDirectSalesProfit(
            $business_id,
            $start_date,
            $end_date,
            $location_id,
        ...
        // Add direct sales profit to gross profit
        $gross_profit = $gross_profit + $direct_sales_profit;
    ...
    /**
     * Auto-create expense entry from purchase or purchase return transaction
     * 
     * @param Transaction $transaction
     * @return Transaction|null
     */
    public function autoCreateExpenseFromPurchase($transaction)
    {
        try {
            ...
            $existing_expense = Transaction::where('business_id', $transaction->business_id)
                ->where('type', 'expense')
                ->where('additional_notes', 'LIKE', '%' . $transaction->ref_no . '%')
                ->where('transaction_date', $transaction->transaction_date)
                ->first();

            if ($existing_expense) {
                // Update existing expense if payment amount changed
                $total_paid = $this->getTotalPaid($transaction->id);
            ...
            $category_name = $transaction->type == 'purchase_return' ? 'Purchase Return' : 'Purchase';
            $expense_category = ExpenseCategory::where('business_id', $transaction->business_id)
            ...
            // For purchase return, use final_total (negative amount)
            if ($transaction->type == 'purchase_return') {
                $total_paid = $transaction->final_total;
            }
            ...
            // Generate reference number
            $ref_count = $this->setAndGetReferenceCount('expense', $transaction->business_id);
            $ref_no = $this->generateReferenceNumber('expense', $ref_count, $transaction->business_id);
            ...
            // Update payment status
            $this->updatePaymentStatus($expense->id, $expense->final_total);
