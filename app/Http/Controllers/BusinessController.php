...
class BusinessController extends Controller
{
    ...
    */
    ...
    public function getBusinessSettings()
    {
        ...
        // Ensure currency_symbol_placement has a default value if not set
        if (empty($business->currency_symbol_placement)) {
    ...
    public function postBusinessSettings(Request $request)
    {
        ...
        try {
            ...
            //Update business settings - Set logo if uploaded
            if (! empty($logo_name)) {
                $business_details['logo'] = $logo_name;
            }
           ...
           // Get existing pos_settings
            ...
            // Handle custom_labels - ensure it exists before encoding
            if (isset($business_details['custom_labels']) && !empty($business_details['custom_labels'])) {
                $business_details['custom_labels'] = json_encode($business_details['custom_labels']);
            } else {
                // Get existing custom_labels from business if not provided
                $existing_business = Business::find($business_id);
                $business_details['custom_labels'] = !empty($existing_business->custom_labels) ? $existing_business->custom_labels : json_encode([]);
            }
            ...
            // Ensure logo is set if uploaded
            if (! empty($logo_name)) {
                $business_details['logo'] = $logo_name;
            }
            ...
            // Explicitly set logo if uploaded (to ensure it's saved)
            if (! empty($logo_name)) {
