...
class HomeController extends Controller
{
    ...
    public function getCalendar()
    {
        ...
        if (request()->ajax()) {
            ...
            // Removed bookings events

            $module_events = $this->moduleUtil->getModuleData('calendarEvents', $data);

            foreach ($module_events as $module_event) {
                // Filter out 'todo' events
                $filtered_events = array_filter($module_event, function($event) {
                    return !isset($event['event_type']) || $event['event_type'] != 'todo';
                });
        ...
        foreach ($module_event_types as $module_event_type) {
            // Filter out 'todo' from event types
            if (isset($module_event_type['todo'])) {
                unset($module_event_type['todo']);
            }
