    ...
    purchase_table = $('#purchase_table').DataTable({
        ...
        "footerCallback": function ( row, data, start, end, display ) {
            ...
            for (var r in data){
                ...
                if (data[r].final_total_raw !== undefined && data[r].final_total_raw !== null) {
                    ...
                    if (final_total_value) {
                        ...
                        // Fallback: try to extract number from text content
                        var text_value = final_total_span.text().replace(/[^\d.-]/g, '');
                        if (text_value) {
                            total_purchase += parseFloat(text_value) || 0;
                        }
                    }
                }
...
function update_grand_total() {
    ...
    //Calculate Discount (fields removed, default to 0)
    var discount_type = $('select#discount_type').length > 0 ? $('select#discount_type').val() : '';
    var discount_amount = $('input#discount_amount').length > 0 ? __read_number($('input#discount_amount'), true) : 0;
    var discount = discount_type ? __calculate_amount(discount_type, discount_amount, total_subtotal) : 0;
    if ($('#discount_calculated_amount').length > 0) {
    ...
    if ($('#tax_id').length > 0 && $('#tax_id').val() && tax === 0) {
        ...
        if ($('#tax_calculated_amount').length > 0) {
            $('#tax_calculated_amount').text(__currency_trans_from_en(tax, true, true));
        }
    }

    //Calculate shipping (fields removed, default to 0)
    var shipping_charges = $('input#shipping_charges').length > 0 ? __read_number($('input#shipping_charges'), true) : 0;
