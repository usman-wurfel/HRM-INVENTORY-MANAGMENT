...
class PayrollController extends Controller
{
            ...
            return Datatables::of($payrolls)
                ->addColumn(
                    ...
                    function ($row) {
                        ...
                        // Get document count
                        $doc_count = \App\Media::where('model_id', $row->id)
                                                ->where('model_type', 'App\Transaction')
                                                ->where('model_media_type', 'payroll_document')
                                                ->count();

                        // Show different button based on permission
                        if (auth()->user()->can('essentials.create_payroll')) {
                            $html .= '<li><a href="#" data-href="'.action([\Modules\Essentials\Http\Controllers\PayrollController::class, 'uploadDocuments'], [$row->id]).'" data-container=".view_modal" class="btn-modal"><i class="fa fa-upload" aria-hidden="true"></i> '.__('lang_v1.upload_docs').'</a></li>';
                        } else {
                            if ($doc_count > 0) {
                                $html .= '<li><a href="#" data-href="'.action([\Modules\Essentials\Http\Controllers\PayrollController::class, 'uploadDocuments'], [$row->id]).'" data-container=".view_modal" class="btn-modal"><i class="fa fa-file" aria-hidden="true"></i> '.__('lang_v1.view_docs').' ('.$doc_count.')</a></li>';
                            }
                        }
        ...
        if (! empty($add_payroll_for)) {
            ...
            foreach ($employees as $employee) {
                ...
                foreach ($allowances_and_deductions as $ad) {
                    ...
                    // Deductions from allowances_and_deductions are not added - only loan deductions are added automatically
                    // else {
                ...
                foreach ($loans as $loan) {
                    ...
                    if ($remaining_loan > 0.001) {
                        ...
                        $payrolls[$employee->id]['deductions']['deduction_readonly'][] = true;
                        $payrolls[$employee->id]['deductions']['deduction_loan_id'][] = $loan->id;
                    }
                }
            }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            foreach ($payrolls as $key => $payroll) {
                ...
                // Store uploaded documents before unsetting
                $uploaded_documents = !empty($payroll['uploaded_documents']) ? $payroll['uploaded_documents'] : null;
                
                unset($payroll['allowance_names'], $payroll['allowance_types'], $payroll['allowance_percent'], $payroll['allowance_amounts'], $payroll['deduction_names'], $payroll['deduction_types'], $payroll['deduction_percent'], $payroll['deduction_amounts'], $payroll['deduction_loan_ids'], $payroll['total'], $payroll['uploaded_documents']);

                $transaction = Transaction::create($payroll);
                $transaction_ids[] = $transaction->id;

                // Attach uploaded documents if file names are provided (from dropzone)
                if (!empty($uploaded_documents)) {
                    $file_names = explode(',', $uploaded_documents);
                    $file_names = array_filter($file_names); // Remove empty values
                    if (!empty($file_names)) {
                        Media::attachMediaToModel($transaction, $business_id, $file_names, null, 'payroll_document');
                    }
                }
        ...
        if (request()->ajax()) {
            try {
                ...
                if ($payroll_group->status == 'draft') {
                    ...
                    // Revert loan deductions for these transactions
                    ...
                    foreach ($transactions as $transaction) {
                        if (!empty($transaction->essentials_deductions)) {
                            ...
                            if (!empty($deductions['deduction_names']) && is_array($deductions['deduction_names'])) {
                                foreach ($deductions['deduction_names'] as $index => $deduction_name) {
                                    $loan_id = !empty($deductions['deduction_loan_ids'][$index]) ? $deductions['deduction_loan_ids'][$index] : null;
                                    $deduction_amount = !empty($deductions['deduction_amounts'][$index]) 
                                        ? $this->moduleUtil->num_uf($deductions['deduction_amounts'][$index]) 
                                        : 0;
                                    
                                    if (!empty($loan_id) && $deduction_amount > 0) {
                                        $loan = EssentialsLoan::where('business_id', $business_id)
                                            ->where('id', $loan_id)
                                            ->first();
                                        
                                        if ($loan) {
                                            $loan->total_deduction_paid = max(0, ($loan->total_deduction_paid ?? 0) - $deduction_amount);
                                            $loan->save();
                                        }
                                    }
                                }
                    ...
                    // Also delete the transactions themselves
        ...
        try {
            ...
            foreach ($payrolls as $key => $payroll) {
                ...
                if (! empty($payroll_trans)) {
                    ...
                    unset($payroll['allowance_names'], $payroll['allowance_types'], $payroll['allowance_percent'], $payroll['allowance_amounts'], $payroll['deduction_names'], $payroll['deduction_types'], $payroll['deduction_percent'], $payroll['deduction_amounts'], $payroll['total'], $payroll['transaction_id'], $payroll['deduction_loan_ids']);
    ...
    public function uploadDocuments($id)
    {
        ...
        $query = Transaction::where('business_id', $business_id)
                        ->with(['media' => function($q) {
                            $q->where('model_media_type', 'payroll_document');
                        }]);
        ...
        $can_upload = auth()->user()->can('essentials.create_payroll');

        return view('essentials::payroll.upload_documents')
            ->with(compact('payroll', 'can_upload'));
    }

    /**
     * Store uploaded documents
     */
    public function postUploadDocuments(Request $request, $id)
    {
        $business_id = request()->session()->get('user.business_id');
        if (! (auth()->user()->can('superadmin') || $this->moduleUtil->hasThePermissionInSubscription($business_id, 'essentials_module'))) {
            abort(403, 'Unauthorized action.');
        ...
        // Check if user can create payroll (only they can upload)
        if (! auth()->user()->can('essentials.create_payroll')) {
            abort(403, 'Unauthorized action.');
        }
        ...
        try {
            ...
            // Upload documents
            Media::uploadMedia($business_id, $payroll, $request, 'documents', false, 'payroll_document');

            $output = [
                'success' => true,
                'msg' => __('lang_v1.documents_uploaded_successfully'),
            ];
        } catch (\Exception $e) {
            \Log::emergency('File:'.$e->getFile().'Line:'.$e->getLine().'Message:'.$e->getMessage());
            $output = [
     ...
     */
    public function deleteDocument($media_id)
    {
        $business_id = request()->session()->get('user.business_id');
        if (! (auth()->user()->can('superadmin') || $this->moduleUtil->hasThePermissionInSubscription($business_id, 'essentials_module'))) {
            abort(403, 'Unauthorized action.');
        ...
        // Check if user can create payroll (only they can delete)
        if (! auth()->user()->can('essentials.create_payroll')) {
            abort(403, 'Unauthorized action.');
        }

        try {
            // Verify the media belongs to a payroll transaction
            ...
            if (! auth()->user()->can('essentials.view_all_payroll') && $transaction->expense_for != auth()->user()->id) {
                abort(403, 'Unauthorized action.');
            }
            ...
            $output = [
                'success' => true,
                'msg' => __('lang_v1.document_deleted_successfully'),
            ];
        } catch (\Exception $e) {
            \Log::emergency('File:'.$e->getFile().'Line:'.$e->getLine().'Message:'.$e->getMessage());
            $output = [
    ...
    public function fixLoanDeductions()
    {
        ...
        try {
            ...
            foreach ($payrolls as $payroll) {
                ...
                $user_name = $payroll->transaction_for->user_full_name ?? $employee_id;
                ...
                foreach ($deductions['deduction_names'] as $index => $deduction_name) {
                    ...
                    if (strpos($deduction_name_lower, 'loan') !== false ||
                        strpos($deduction_name_lower, 'advance') !== false || // Added "advance"
                        strpos($deduction_name_lower, 'empréstimo') !== false || 
                        preg_match('/\(L\d+\)/', $deduction_name)) { 
                        $is_loan_deduction = true;
                    }
                    
                    $matched_loan_id_by_name = null;
                    
                    // Always try to match by REF even if "loan" keyword is missing
                    if (!isset($employee_loans_cache[$employee_id])) {
                        $employee_loans_cache[$employee_id] = EssentialsLoan::where('business_id', $business_id)
                            ->where('user_id', $employee_id)
                            ->get();
                    }
                    
                    foreach ($employee_loans_cache[$employee_id] as $el) {
                        if (!empty($el->ref_no) && stripos($deduction_name, $el->ref_no) !== false) {
                            $is_loan_deduction = true;
                            $matched_loan_id_by_name = $el->id;
                            break;
                        }
                    }
                    
                    if (!$is_loan_deduction) {
                        $debug_log[] = "Skipped (Not Loan): User: $user_name, Ded: '$deduction_name', Amt: $deduction_amount";
                        continue;
                    }

                    $loan_id = null;
                    
                    // If we matched by specific reference in the name, use that!
                    if (!empty($matched_loan_id_by_name)) {
                        $loan_id = $matched_loan_id_by_name;
                    }

                    // First, try to get loan_id from stored deduction_loan_ids
                    if (empty($loan_id) && !empty($deductions['deduction_loan_ids'][$index])) {
                        $loan_id = $deductions['deduction_loan_ids'][$index];
                    }
                    // Second: try to match by strict format "Loan (REF)"
                    if (empty($loan_id) && preg_match('/\s*\(([^)]+)\)\s*$/', $deduction_name, $ref_match)) {
                        $loan_ref = trim($ref_match[1]);
                        foreach ($employee_loans_cache[$employee_id] as $el) {
                            if ($el->ref_no == $loan_ref) {
                                $loan_id = $el->id;
                                break;
                            }
                        }
                    ...
                    if (!empty($loan_id)) {
                        ...
                        $current_total = $loan_deductions[$loan_id];
                        // Find the loan object again to get loan_amount
                        $loan_obj = $employee_loans_cache[$employee_id]->where('id', $loan_id)->first();
                        $total_loan_amount = $loan_obj ? $loan_obj->loan_amount : 999999999;

                        $remaining_capacity = $total_loan_amount - $current_total;
                        
                        $amount_to_add = min($deduction_amount, $remaining_capacity);
            ...
            $output = [
                'success' => true,
                'msg' => "Fixed: $fixed_count, Skipped: $skipped_count. <br>Start Log:<br>" . implode("<br>", array_slice($debug_log, -20)) // Show last 20 logs
            ];
        } catch (\Exception $e) {
            DB::rollBack();
            \Log::emergency('File:'.$e->getFile().'Line:'.$e->getLine().'Message:'.$e->getMessage());
