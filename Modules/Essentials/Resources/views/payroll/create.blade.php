...
{!! Form::open(['url' => $action_url, 'method' => 'post', 'id' => 'add_payroll_form', 'enctype' => 'multipart/form-data' ]) !!}
                        ...
                        @foreach($payrolls as $employee => $payroll)
                                            ...
                                            <tbody>
                                                ...
                                                @if(!empty($payroll['deductions']))
                                                    @foreach($payroll['deductions']['deduction_names'] as $key => $value)
                                                        @php
                                                            $is_readonly = !empty($payroll['deductions']['deduction_readonly'][$key]) ? true : false;
                                                            $loan_id = !empty($payroll['deductions']['deduction_loan_id'][$key]) ? $payroll['deductions']['deduction_loan_id'][$key] : null;
                                                        @endphp
                                                        @include('essentials::payroll.allowance_and_deduction_row', ['add_button' => false, 'type' => 'deduction', 'name' => $value, 'value' => $payroll['deductions']['deduction_amounts'][$key], 
                                                        'amount_type' => $payroll['deductions']['deduction_types'][$key], 'percent' => $payroll['deductions']['deduction_percents'][$key], 'is_readonly' => $is_readonly, 'loan_id' => $loan_id])
                                                    ...
                                                    {{-- Add empty rows with add button for manual deductions --}}
                                                    @include('essentials::payroll.allowance_and_deduction_row', ['add_button' => true, 'type' => 'deduction', 'is_readonly' => false, 'loan_id' => null])
                                                    @include('essentials::payroll.allowance_and_deduction_row', ['type' => 'deduction', 'is_readonly' => false, 'loan_id' => null])
                                                    @include('essentials::payroll.allowance_and_deduction_row', ['type' => 'deduction', 'is_readonly' => false, 'loan_id' => null])
                                                @else
                                                    @include('essentials::payroll.allowance_and_deduction_row', ['add_button' => true, 'type' => 'deduction', 'is_readonly' => false, 'loan_id' => null])
                                                    @include('essentials::payroll.allowance_and_deduction_row', ['type' => 'deduction', 'is_readonly' => false, 'loan_id' => null])
                            ...
                            <tr>
                                <td colspan="5">
                                    ...
                                    <div class="form-group">
                                        {!! Form::label('documents_'.$employee, __( 'lang_v1.upload_documents' ) . ':') !!}
                                        <div class="dropzone" id="payroll_documents_dropzone_{{$employee}}"></div>
                                        <input type="hidden" name="payrolls[{{$employee}}][uploaded_documents]" id="uploaded_documents_{{$employee}}" value="">
                                        <small class="help-block">@lang('lang_v1.you_can_upload_multiple_files')</small>
                                    </div>
                                </td>
                            </tr>
                        @endforeach
