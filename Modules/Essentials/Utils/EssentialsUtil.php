...
class EssentialsUtil extends Util
{
    ...
    public function Gettotalholiday($business_id, $location, $start_date, $end_date, $permitted_locations){
        $holidays = EssentialsHoliday::where('essentials_holidays.business_id', $business_id)
                        ...
                        ->leftJoin('users as u', 'u.id', '=', 'essentials_holidays.user_id')
                        ->select([
                            'essentials_holidays.id',
                            'essentials_holidays.name',
                            'essentials_holidays.type',
                            'bl.name as location',
                            'essentials_holidays.start_date',
                            'essentials_holidays.end_date',
                            'essentials_holidays.weekdays',
                            'essentials_holidays.repeat_type',
                            'essentials_holidays.repeat_days',
            ...
            // For normal holidays, filter by date range
            if (! empty($start_date) && ! empty($end_date)) {
                $holidays->where(function($query) use ($start_date, $end_date) {
                    $query->where(function($q) use ($start_date, $end_date) {
                        // Normal holidays with date range
                        $q->where('essentials_holidays.type', 'normal')
                          ->whereDate('essentials_holidays.start_date', '>=', $start_date)
                          ->whereDate('essentials_holidays.start_date', '<=', $end_date);
                    })->orWhere('essentials_holidays.type', 'consecutive');
                    // Consecutive holidays - show all (they don't have start_date)
                });
            }
